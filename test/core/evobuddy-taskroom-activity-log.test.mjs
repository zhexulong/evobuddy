...
test('handoff writes activity evidence for handoff and wake', async () => {
  ...
  const room = await createTaskRoomFromDraft(projectRoot, { objective: 'activity evidence' });
  ...
  assert.ok(events.some((event) => event.kind === 'handoff'));
  assert.ok(events.some((event) => event.kind === 'wake' && event.wakeReason === 'handoff-ready'));
  assert.ok(events.every((event) => event.body === undefined));
});
